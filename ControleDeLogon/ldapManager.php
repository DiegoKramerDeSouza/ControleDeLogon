...
<html>
	...
	<?php
		...
		if(isset($_POST['searchUser']) && isset($_POST['database_user'])){
			$strUser = $_POST['searchUser'];
			$returnbase = $_POST['database_user'];
			$base = $returnbase;
		} else {
			$base = $dbView;
			$returnbase = $base;
		...
		//========================================================================================================
			...
			for ($i = 0; $i < $info["count"]; $i++) {		
				...
				if (isset($info[$i]["thumbnailphoto"])){
					...
					$picture = "<img class='img-circle' src='data:image/jpeg;base64," . base64_encode($picture) . "' style='width:80px; height:80px;'/>";
				} else {
					$picture = "<img class='img-circle' src='Images/user_icon.png' style='width:80px; height:80px;'>";
				}
				...
				if ($unlock >= 3){
					if ($accountControl == 66050){
						$status = "<a href='#' id='userDisabled' style='color:#555;'><b>Desabilitada</b></a> e <a href='http://10.61.200.111/ldap/unlock.php?searchUser=" . $strUser . "&database_unlock=" . $returnbase . "' style='color:#e00;'>Bloqueada</a></span>";
					} else {
						$status = "Habilitada e <a href='http://10.61.200.111/ldap/unlock.php?searchUser=" . $strUser . "&database_unlock=" . $returnbase . "' style='color:#e00;'>Bloqueada</a></span>";
					}
				} else {
					if ($accountControl == 66050){
				...
				$editBtn = "<button type='button' id='EditModal' class='btn btn-success btn-lg editBtn' data-toggle='modal' data-target='#editModal' onclick='clearValues()'>".
			...
			//==============================================================
				...
				$html = $html.
						"<div class='container information' id='" . $account . "_unlock'>".
